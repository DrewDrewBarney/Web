...
include '../../RunningSite/Pages/ZwiftBase.php';

/*
// image banner
include_once '../ClubPHP/ClubTools.php';
$body->addChild(makeClubBanner("../ClubImages/Zwift.png"));

//$body->makeChild("img", "", ["src" => "../Images/Zwift.png", "alt" => "Critical", "style" => "float:right; margin: 5ch 17ch 5ch 5ch; width: 20%;"]);
...
$form = $schemaArticle->makeChild('form', '', ['id' => 'form']);
...
$last10kTime = $form->makeChild('div', '', $commonDivStyle)->makeChild('input', ' last 10k time', ['name' => 'last10kTime', 'value' => '53', 'type' => 'string']);
...
for ($i = 0; $i < $numberOfPhases; $i++) {
    ...
    $name = "select_$i";
    $select = $row->makeChild('td')->makeChild('select', '', ['name' => $name, 'onchange' => "document.getElementById('form').submit();"]);
    $controls[$name] = $select;
    foreach ($phaseTypes as $phaseType => $zwiftPhaseType) {
        $option = $select->makeChild('option', $phaseType);
        if ($phaseType === $select->value()) {
    ...
    switch ($select->value()) {
        ...
        case 'Steady State':
            ...
            // the allure
            $controls["allure_$i"] = $row->makeChild('td')->makeChild('input', ' as', ['type' => 'number', 'name' => "allure_$i", 'min' => '1', 'max' => '1000', 'value' => '10', 'onchange' => "document.getElementById('form').submit();"]);
...
$createButton = $zwiftArticle->makeChild('div', '', $commonDivStyle)->makeChild('button', 'Create', ['name' => 'create', 'value' => 'create', 'form' => 'form']);
...
if ($createButton->value()) {
    ...
    $pre = $body->makeChild('pre', '', ['class' => 'ZwiftWorkoutCard']);
    ...
    $pre->addChild($work);
     ...
     // copy to clipboard
    $body->makeChild('div', $workoutString, ['id' => 'workout', 'style' => 'display:none;']);
    $body->makeChild('div', '', ['id' => 'workout', 'style' => 'padding:0ch 0ch 3ch 4ch;'])
            ->makeChild('button', "Copy to clipboard", [
                'onclick' =>
                "const workout = document.getElementById('workout');" .
                'navigator.clipboard.writeText(workout.innerHTML);']);
}
...
$createTestButton = $zwiftArticle->makeChild('div', '', $commonDivStyle)->makeChild('button', 'Create', ['name' => 'createTest', 'value' => 'createTest', 'form' => 'form']);

if ($createTestButton->value()) {
