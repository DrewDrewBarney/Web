...
<html>
    ...
    <?php
    makeHead('Objective Measures')->echo();
    ?>
    ...
    <body>
        ...
        $topBar->addChildren([
            ...
            makeMenu(MENU_BASIS, 'metrics.php'),
            makePageTitle('Objective Measures of Training Intensity', ['home' => 'index.php'])
        ]);
        $topBar->echo();
        ?>
        ...
        <article>
            ...
            $table = new SimpleTable([
                ['Measure', 'Responsive', 'Drift Free', 'Gradient Independent', 'Device Independent', 'Wind Independent'],
                ['Pace', 'YES', 'YES', 'NO', 'YES', 'NO'],
                ['HR', 'NO', 'NO', 'NO', 'YES', 'YES'],
                ['Power', 'YES', 'YES', 'YES', 'NO', 'SOME DEVICES'],
                ['Power GAP', 'YES', 'YES', 'YES', 'YES', 'SOME DEVICES'],
            ]);
            $table->addCaption('Comparison of Metrics');
            //$table->setTableStyle('width', '60ch');
            echo $table->toString();
            ?>
            ...
            <p>
                Running power is now readily available on many devices. 
                Unfortunately it is not in the commercial interest of companies implementing running power to agree upon a common standard. 
                Clearly the best option is to convert any proprietary power number to a common currency, power based gradient adjusted pace. 
                We might call this pGAP. Doing so and adopting pGAP would make data from different devices comparable and understandable. 
                pGAP on a running track without wind would however merely be pace and the marketing bubble might burst!
                
             </p><p>
               
                The end result is that if runners are not careful, their running power data sets will be polluted by significantly 
                different values from different devices (e.g. wrist based power on a Garmin watch vs. Stryd) and any analysis based on these mixed data sets worthless.
